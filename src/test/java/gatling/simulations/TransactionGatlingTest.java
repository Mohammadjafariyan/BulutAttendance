 ...
 */
public class TransactionGatlingTest extends Simulation {
        ...
        .on(
            exec(
                http("Get all transactions")
                    .get("/services/bulutattendance/api/transactions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new transaction")
                        .post("/services/bulutattendance/api/transactions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created transaction")
                            .get("/services/bulutattendance${new_transaction_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created transaction")
                        .delete("/services/bulutattendance${new_transaction_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Transaction entity").exec(scn);
