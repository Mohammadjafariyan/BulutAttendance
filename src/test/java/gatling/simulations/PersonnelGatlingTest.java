...
public class PersonnelGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all personnel")
                    .get("/services/bulutattendance/api/personnel")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new personnel")
                        .post("/services/bulutattendance/api/personnel")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"requitmentDate\": \"SAMPLE_TEXT\"" +
                                ", \"father\": \"SAMPLE_TEXT\"" +
                                ", \"shenasname\": \"SAMPLE_TEXT\"" +
                                ", \"mahalesodur\": \"SAMPLE_TEXT\"" +
                                ", \"birthday\": \"SAMPLE_TEXT\"" +
                                ", \"isSingle\": \"SAMPLE_TEXT\"" +
                                ", \"lastEducation\": \"SAMPLE_TEXT\"" +
                                ", \"educationField\": \"SAMPLE_TEXT\"" +
                                ", \"children\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_personnel_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created personnel")
                            .get("/services/bulutattendance${new_personnel_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created personnel")
                        .delete("/services/bulutattendance${new_personnel_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Personnel entity").exec(scn);
