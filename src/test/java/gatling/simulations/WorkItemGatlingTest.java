...
public class WorkItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all workItems")
                    .get("/services/bulutattendance/api/work-items")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new workItem")
                        .post("/services/bulutattendance/api/work-items")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"amount\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_workItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created workItem").get("/services/bulutattendance${new_workItem_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created workItem")
                        .delete("/services/bulutattendance${new_workItem_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the WorkItem entity").exec(scn);
