...
public class TaxTemplateGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all taxTemplates")
                    .get("/services/bulutattendance/api/tax-templates")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new taxTemplate")
                        .post("/services/bulutattendance/api/tax-templates")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"rangeFrom\": 0" + ", \"rangeTo\": 0" + ", \"percent\": 0" + ", \"year\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_taxTemplate_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created taxTemplate")
                            .get("/services/bulutattendance${new_taxTemplate_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created taxTemplate")
                        .delete("/services/bulutattendance${new_taxTemplate_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TaxTemplate entity").exec(scn);
