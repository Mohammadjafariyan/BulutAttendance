...
public class HrLetterParameterGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all hrLetterParameters")
                    .get("/services/bulutattendance/api/hr-letter-parameters")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new hrLetterParameter")
                        .post("/services/bulutattendance/api/hr-letter-parameters")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_hrLetterParameter_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created hrLetterParameter")
                            .get("/services/bulutattendance${new_hrLetterParameter_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created hrLetterParameter")
                        .delete("/services/bulutattendance${new_hrLetterParameter_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the HrLetterParameter entity").exec(scn);
