...
public class AccProcStepGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all accProcSteps")
                    .get("/services/bulutattendance/api/acc-proc-steps")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new accProcStep")
                        .post("/services/bulutattendance/api/acc-proc-steps")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_accProcStep_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created accProcStep")
                            .get("/services/bulutattendance${new_accProcStep_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created accProcStep")
                        .delete("/services/bulutattendance${new_accProcStep_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AccProcStep entity").exec(scn);
