...
public class LeaveSummaryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all leaveSummaries")
                    .get("/services/bulutattendance/api/leave-summaries")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new leaveSummary")
                        .post("/services/bulutattendance/api/leave-summaries")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"remainHours\": 0" + ", \"remainDays\": 0" + ", \"year\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_leaveSummary_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created leaveSummary")
                            .get("/services/bulutattendance${new_leaveSummary_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created leaveSummary")
                        .delete("/services/bulutattendance${new_leaveSummary_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LeaveSummary entity").exec(scn);
