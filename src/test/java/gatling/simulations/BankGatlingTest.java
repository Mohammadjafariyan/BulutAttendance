 ...
 */
public class BankGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all banks").get("/services/bulutattendance/api/banks").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new bank")
                        .post("/services/bulutattendance/api/banks")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"title\": \"SAMPLE_TEXT\"" + ", \"code\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_bank_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created bank").get("/services/bulutattendance${new_bank_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created bank").delete("/services/bulutattendance${new_bank_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Bank entity").exec(scn);
