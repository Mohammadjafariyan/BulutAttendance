...
/**
 * Performance test for the AccProcStepExecution entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class AccProcStepExecutionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all accProcStepExecutions")
                    .get("/services/bulutattendance/api/acc-proc-step-executions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new accProcStepExecution")
                        .post("/services/bulutattendance/api/acc-proc-step-executions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"debitAmount\": 0" + ", \"creditAmount\": 0" + ", \"desc\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_accProcStepExecution_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created accProcStepExecution")
                            .get("/services/bulutattendance${new_accProcStepExecution_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created accProcStepExecution")
                        .delete("/services/bulutattendance${new_accProcStepExecution_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AccProcStepExecution entity").exec(scn);
