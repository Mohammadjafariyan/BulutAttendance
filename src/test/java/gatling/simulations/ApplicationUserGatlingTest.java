        ...
        .on(
            exec(
                http("Get all applicationUsers")
                    .get("/services/bulutattendance/api/application-users")
                ...
                .exec(
                    http("Create new applicationUser")
                        .post("/services/bulutattendance/api/application-users")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"userId\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .on(
                    exec(
                        http("Get created applicationUser")
                            .get("/services/bulutattendance${new_applicationUser_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created applicationUser")
                        .delete("/services/bulutattendance${new_applicationUser_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
