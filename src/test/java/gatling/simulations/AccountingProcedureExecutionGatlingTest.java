...
/**
 * Performance test for the AccountingProcedureExecution entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class AccountingProcedureExecutionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all accountingProcedureExecutions")
                    .get("/services/bulutattendance/api/accounting-procedure-executions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new accountingProcedureExecution")
                        .post("/services/bulutattendance/api/accounting-procedure-executions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"dateTime\": \"2020-01-01T00:00:00.000Z\"" + ", \"desc\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_accountingProcedureExecution_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created accountingProcedureExecution")
                            .get("/services/bulutattendance${new_accountingProcedureExecution_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created accountingProcedureExecution")
                        .delete("/services/bulutattendance${new_accountingProcedureExecution_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AccountingProcedureExecution entity").exec(scn);
