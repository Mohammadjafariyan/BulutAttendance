...
public class AccountTemplateGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all accountTemplates")
                    .get("/services/bulutattendance/api/account-templates")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new accountTemplate")
                        .post("/services/bulutattendance/api/account-templates")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"code\": \"SAMPLE_TEXT\"" +
                                ", \"level\": 0" +
                                ", \"levelTitle\": \"SAMPLE_TEXT\"" +
                                ", \"type\": \"Credit\"" +
                                ", \"levelInTree\": \"GROUP\"" +
                                ", \"debitAmount\": 0" +
                                ", \"creditAmount\": 0" +
                                ", \"typeInFormula\": \"ASSETS\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_accountTemplate_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created accountTemplate")
                            .get("/services/bulutattendance${new_accountTemplate_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created accountTemplate")
                        .delete("/services/bulutattendance${new_accountTemplate_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AccountTemplate entity").exec(scn);
