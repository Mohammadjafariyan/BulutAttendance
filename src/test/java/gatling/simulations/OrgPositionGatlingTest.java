...
public class OrgPositionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all orgPositions")
                    .get("/services/bulutattendance/api/org-positions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new orgPosition")
                        .post("/services/bulutattendance/api/org-positions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"title\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_orgPosition_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created orgPosition")
                            .get("/services/bulutattendance${new_orgPosition_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created orgPosition")
                        .delete("/services/bulutattendance${new_orgPosition_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrgPosition entity").exec(scn);
