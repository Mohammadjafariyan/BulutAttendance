 ...
 */
public class WorkGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all works").get("/services/bulutattendance/api/works").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new work")
                        .post("/services/bulutattendance/api/works")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_work_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created work").get("/services/bulutattendance${new_work_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created work").delete("/services/bulutattendance${new_work_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Work entity").exec(scn);
