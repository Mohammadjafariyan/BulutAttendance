...
public class SysConfigGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all sysConfigs")
                    .get("/services/bulutattendance/api/sys-configs")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new sysConfig")
                        .post("/services/bulutattendance/api/sys-configs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"taxFormula\": \"SAMPLE_TEXT\"" + ", \"sanavatFormula\": \"SAMPLE_TEXT\"" + ", \"year\": 0" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_sysConfig_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created sysConfig")
                            .get("/services/bulutattendance${new_sysConfig_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created sysConfig")
                        .delete("/services/bulutattendance${new_sysConfig_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SysConfig entity").exec(scn);
