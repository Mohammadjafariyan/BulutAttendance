...
public class PersonnelStatusGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all personnelStatuses")
                    .get("/services/bulutattendance/api/personnel-statuses")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new personnelStatus")
                        .post("/services/bulutattendance/api/personnel-statuses")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"title\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_personnelStatus_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created personnelStatus")
                            .get("/services/bulutattendance${new_personnelStatus_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created personnelStatus")
                        .delete("/services/bulutattendance${new_personnelStatus_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PersonnelStatus entity").exec(scn);
