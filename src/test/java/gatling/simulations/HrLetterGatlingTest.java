...
public class HrLetterGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all hrLetters")
                    .get("/services/bulutattendance/api/hr-letters")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new hrLetter")
                        .post("/services/bulutattendance/api/hr-letters")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"title\": \"SAMPLE_TEXT\"" +
                                ", \"uniqueNumber\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_hrLetter_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created hrLetter").get("/services/bulutattendance${new_hrLetter_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created hrLetter")
                        .delete("/services/bulutattendance${new_hrLetter_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the HrLetter entity").exec(scn);
