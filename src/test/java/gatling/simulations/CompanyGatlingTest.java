        ...
        .on(
            exec(
                http("Get all companies")
                    .get("/services/bulutattendance/api/companies")
                ...
                .exec(
                    http("Create new company")
                        .post("/services/bulutattendance/api/companies")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"title\": \"SAMPLE_TEXT\"" + ", \"logo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .on(
                    exec(
                        http("Get created company").get("/services/bulutattendance${new_company_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created company").delete("/services/bulutattendance${new_company_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );
