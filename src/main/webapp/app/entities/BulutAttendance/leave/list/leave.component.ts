...
import { EntityArrayResponseType, LeaveService } from '../service/leave.service';
...
export class LeaveComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILeave): string => this.leaveService.getLeaveIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && LeaveComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && LeaveComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.leaves = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ILeave[] | null): ILeave[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.leaveService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.leaveService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
