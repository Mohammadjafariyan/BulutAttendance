...
import { EntityArrayResponseType, PersonnelService } from '../service/personnel.service';
...
export class PersonnelComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPersonnel): string => this.personnelService.getPersonnelIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && PersonnelComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && PersonnelComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.personnel = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPersonnel[] | null): IPersonnel[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.personnelService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.personnelService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
