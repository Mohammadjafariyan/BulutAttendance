...
export class TransactionAccountService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/transaction-accounts', 'bulutattendance');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/transaction-accounts/_search', 'bulutattendance');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    ...
    return this.http
      ...
      .pipe(catchError(() => scheduled([new HttpResponse<ITransactionAccount[]>()], asapScheduler)));
  }
  ...
  addTransactionAccountToCollectionIfMissing<Type extends Pick<ITransactionAccount, 'id'>>(
    ...
    if (transactionAccounts.length > 0) {
      const transactionAccountCollectionIdentifiers = transactionAccountCollection.map(transactionAccountItem =>
        this.getTransactionAccountIdentifier(transactionAccountItem),
      );
