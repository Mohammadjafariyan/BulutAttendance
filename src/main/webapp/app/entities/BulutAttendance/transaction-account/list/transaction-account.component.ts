...
import { EntityArrayResponseType, TransactionAccountService } from '../service/transaction-account.service';
...
export class TransactionAccountComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITransactionAccount): number => this.transactionAccountService.getTransactionAccountIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.transactionAccounts = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITransactionAccount[] | null): ITransactionAccount[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.transactionAccountService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.transactionAccountService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
