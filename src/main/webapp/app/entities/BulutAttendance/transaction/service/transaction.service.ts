...
type RestOf<T extends ITransaction | NewTransaction> = Omit<T, 'issueDateTime'> & {
...
export class TransactionService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/transactions', 'bulutattendance');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/transactions/_search', 'bulutattendance');
  ...
  protected convertDateFromClient<T extends ITransaction | NewTransaction | PartialUpdateTransaction>(transaction: T): RestOf<T> {
    return {
      ...transaction,
      issueDateTime: transaction.issueDateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTransaction: RestTransaction): ITransaction {
    return {
      ...restTransaction,
      issueDateTime: restTransaction.issueDateTime ? dayjs(restTransaction.issueDateTime) : undefined,
    };
  }
