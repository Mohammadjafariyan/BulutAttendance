 ...
 */
type FormValueOf<T extends ITransaction | NewTransaction> = Omit<T, 'issueDateTime'> & {
...
type TransactionFormDefaults = Pick<NewTransaction, 'id' | 'issueDateTime'>;

type TransactionFormGroupContent = {
  id: FormControl<TransactionFormRawValue['id'] | NewTransaction['id']>;
  issueDateTime: FormControl<TransactionFormRawValue['issueDateTime']>;
  totalAmount: FormControl<TransactionFormRawValue['totalAmount']>;
  desc: FormControl<TransactionFormRawValue['desc']>;
  docNumber: FormControl<TransactionFormRawValue['docNumber']>;
  internalUser: FormControl<TransactionFormRawValue['internalUser']>;
  company: FormControl<TransactionFormRawValue['company']>;
};
...
export class TransactionFormService {
  createTransactionFormGroup(transaction: TransactionFormGroupInput = { id: null }): TransactionFormGroup {
    ...
    return new FormGroup<TransactionFormGroupContent>({
      ...
      issueDateTime: new FormControl(transactionRawValue.issueDateTime),
      totalAmount: new FormControl(transactionRawValue.totalAmount),
      desc: new FormControl(transactionRawValue.desc),
      docNumber: new FormControl(transactionRawValue.docNumber),
      internalUser: new FormControl(transactionRawValue.internalUser),
      company: new FormControl(transactionRawValue.company),
    });
  }
  ...
  private convertTransactionRawValueToTransaction(
    ...
    return {
      ...rawTransaction,
      issueDateTime: dayjs(rawTransaction.issueDateTime, DATE_TIME_FORMAT),
    };
  }
  ...
  private convertTransactionToTransactionRawValue(
    ...
    return {
      ...transaction,
      issueDateTime: transaction.issueDateTime ? transaction.issueDateTime.format(DATE_TIME_FORMAT) : undefined,
    };
  }
}
